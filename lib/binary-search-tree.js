...
export default class BST {
  ...
  each(fn, order='in', node) {
    ...
    // const inorder = [node.left, node, node.right];
    // const preorder = [node, node.left, node.right];
    // const postorder = [node.left, node.right, node];

    // order.forEach(x => {
    //   x === node 
    //     ? fn(x)
    //     : each(fn, node.left, order);
    // })

    switch(order) {
      case 'pre': {
        fn(node);
        this.each(fn, order, node.left);
        this.each(fn, order, node.right);
        break;
      };
